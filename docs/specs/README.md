...
docs/specs/
├── README.md (このファイル)
├── task-management/                # タスク管理機能（コア仕様書）⭐ NEW
│   ├── README.md
│   ├── 01-data-structure.md
│   ├── 02-wbs-numbering-system.md
│   └── 03-deletion-policy.md
...
**ステータス**: ✅ 実装済み（v2.13.4）、⚠️ バグ修正待ち
**バージョン**: 2.0
**工数見積もり**: バグ修正 20-25分

タスク管理のコア機能（データ構造、WBS番号体系、削除ポリシー）を定義します。

#### 仕様書一覧
- [README.md](./task-management/README.md) - 全体概要、次のアクション
- [01-data-structure.md](./task-management/01-data-structure.md) - タスクデータ構造（JSON Schema、全フィールド定義）
...
- ✅ タスクデータ構造定義（v2.0）
- ✅ 論理削除実装（v2.13.0）
- ⚠️ **WBS番号自動生成バグあり**（Major/Minor抽出エラー - Opus指摘）
- ⚠️ **削除後ID再利用問題あり**（Option 1実装待ち - 1行変更）

#### 外部AI評価
- ✅ Claude Opus、GPT、Gemini による詳細評価完了
- ✅ 全AI一致の推奨: Option 1（削除済みID除外 + 全履歴考慮）

#### 次のアクション
🔴 **最優先**: WBS番号バグ修正 + Option 1実装（20-25分）

詳細: [archive/session/03_ID-policy/](../archive/session/03_ID-policy/)
...
| 機能 | 優先度 | 理由 |
|------|--------|------|
| **タスク管理（バグ修正）** | 🔴 **緊急** | WBS番号バグ + Option 1実装（工数20-25分） |
...
| 日付 | 変更内容 |
|------|---------|
| 2026-03-22 | タスク管理コア仕様書追加（task-management/）、優先順位テーブル更新 |
